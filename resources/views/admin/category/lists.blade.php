...
@section('content')
    <section class="content-header">
        <h1>
            <a class="btn bg-gray-active btn-sm" href="{{url("/admin/category")}}">类型列表</a>
            @if(Auth::guard('admin')->user()->can('categoryAdd'))
                <a class="btn btn-linkedin btn-sm" href="{{url("/admin/category/create")}}">新增类型</a>
            @endif
        </h1>
        <ol class="breadcrumb">
            ...
            <div class="col-xs-12">
                <div class="box">
                    <label style="width: 110px;">
                        <select class="form-control select2" onchange="change(this);">
                                    ...
                                    <td>
                                        @if(Auth::guard('admin')->user()->can('categoryEdit'))
                                            <a href="{{url("/admin/category/"."$value->id".'/edit')}}">
                                                <button class="btn btn-sm bg-yellow-active pull-left margin-r-5">修改
                                                </button>
                                            </a>
                                        @endif
                                        @if(Auth::guard('admin')->user()->can('categoryDelete'))
                                            <button onclick="deleteIt('/admin/category/delete?id={{$value->id}}')"
                                                    class="btn btn-adn btn-sm">删除
                                            </button>
                                        @endif
                                    </td>
                                </tr>
