...
@section('content')
    <section class="content-header">
        <h1>
            <a class="btn bg-gray-active btn-sm" href="{{url("/admin/admins")}}">管理员列表</a>
            @if(Auth::guard('admin')->user()->can('adminAdd'))
                <a class="btn btn-linkedin btn-sm" href="{{url("/admin/admins/create")}}">新增管理员</a>
            @endif
        </h1>
        <ol class="breadcrumb">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if(Auth::guard('admin')->user()->can('adminDelete') && $value->id != 1)
                                            <button onclick="deleteIt('/admin/admins/delete?id={{$value->id}}')"
                                                    class="btn btn-sm btn-adn">删除
                                            </button>
                                        @endif
                                    </td>
                                </tr>
